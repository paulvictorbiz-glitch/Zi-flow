...
- **The public front page `/` (www.footagebrain.com) is the Paul Victor cinematic portfolio (LIVE 2026-07-05, `8507797`; updated 2026-07-06, `afb132b`)** — served as its OWN isolated Vite build in a full-viewport iframe (`src/pages/portfolio-front.jsx` → `/portfolio/index.html`; files under `public/portfolio/`). The app + every tab are UNCHANGED at `/app`. The old Reel DNA marketing landing (`src/pages/landing.jsx`) is **ROUTED AGAIN at `/landing`** (2026-07-06, so the portfolio's "Book a call" CTA has somewhere to link — was: preserved-but-unrouted). All three of the portfolio's video-like clips (hero orbit + builder + closer) are canvas frame-sequences scroll-scrubbed off `ScrollTrigger` progress — none of them are real `<video>` playback. Portfolio SOURCE is the SEPARATE repo `C:\Users\Mi\Downloads\paulvictor-portfolio`; to update: build there → copy `dist/*` into `public/portfolio/` → deploy. See memory `project_footagebrain-portfolio-front.md` · `project_paulvictor-portfolio.md`.
