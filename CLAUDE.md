...
1. **Deploy = `vercel --prod` only.** `git push` does NOT deploy. Never assume a push updated the live site. **`vercel --prod` builds the ENTIRE working tree — every uncommitted change ships, not just the files you touched.** The owner manages the dirty tree and decides what's ready — do NOT run pre-deploy tree-conflict checks, flag dirty files, or ask for reconciliation. Just deploy when asked. See memory `feedback_full-tree-deploy.md`. **One gotcha on ISOLATED deploys (stash dirty WIP → build from clean `main` → deploy):** the bundle ships NONE of the stashed files — so if a dirty file was *live-but-never-committed*, stashing **silently reverts it on prod**. Only flag this specific scenario (isolated stash deploy) if it's clearly about to happen. See memory `reference_isolated-deploy-reverts-live-uncommitted.md`.
...
- **The public front page `/` (www.footagebrain.com) is the old Reel DNA marketing landing again** (`src/pages/landing.jsx`, `/landing` is an alias) — **REVERTED 2026-07-06** after a one-day stint as the Paul Victor cinematic portfolio (LIVE 2026-07-05 `8507797`, updated `afb132b`). The portfolio now lives at **`/3d`**, reachable only via a hidden easter egg (the period after "See the DNA of any reel." in the hero headline is a disguised `<button class="lp-easter-dot">`, `all: unset` styling, no visible link) with a floating "← Back" returning to `/`. Still served as its OWN isolated Vite build in a full-viewport iframe (`src/pages/portfolio-front.jsx` → `/portfolio/index.html`; files under `public/portfolio/`). The app + every tab are UNCHANGED at `/app`. All three of the portfolio's video-like clips (hero orbit + builder + closer) are canvas frame-sequences scroll-scrubbed off `ScrollTrigger` progress — none of them are real `<video>` playback. Portfolio SOURCE is the SEPARATE repo `C:\Users\Mi\Downloads\paulvictor-portfolio`; to update: build there → copy `dist/*` into `public/portfolio/` → deploy. See memory `project_footagebrain-portfolio-front.md` · `project_paulvictor-portfolio.md`.
- **OpenCut 3-editor durability Phase 1 (BUILT 2026-07-06, both build gates green, NOT deployed):** shared content-addressed media (`oc-media` bucket + `oc_media` dedup ledger, migration `0112_oc_media.sql`) + CAS-checked autosave fix the two real gaps for 3 simultaneous editors — media going silently black when a project opens on another machine, and a solo-editor save/reload-loss bug. Spans TWO repos (FB = migration 0112 only; fork `C:\Users\Mi\Downloads\opencut-ai` = all editor code + new `hash-wasm` dep). To go live: apply 0112 (scoped one-off) + rebuild the fork's Hetzner image — neither done. Owner also gates a Supabase-Pro-vs-Hetzner-media call after a trial. See memory `project_opencut-3editor-durability.md`, runbook `docs/opencut-durability-deploy.md`.
...
- **`src/pages/fonts.jsx` + `src/fonts.css` is the owner-only "Fonts" (Font ID) tab** (SHIPPED LIVE 2026-07-03, Monitor group) — identifies the typeface in a reel frame (screenshot upload/paste OR reel-URL keyframes) → top-3 fonts + confidence + LEGIT-only download links. Backend = `content_forge.py` `_forge_vision` + `POST /content-forge/font-id[-keyframes]` (Gemini vision on paid Vertex 2.5-flash); proxy = `api/ai/suggest.js?action=font-id`. Gated purely on `isOwner` like Content Forge (NOT in `permissions-catalog.js`). Rides Content Forge's kill switch + `kind="font_id"` usage telemetry. Phase 2 (ONNX + WhatFontIs) planned. See memory `project_font-id-feature.md` + `reference_suggest-js-auth-gate.md`.
...
- **`DEADWEIGHT.md` (repo root) is the trim register** (NEW 2026-06-27) — when building anything dormant/heavy/redundant, tag it there at creation + drop a grep-able `DEADWEIGHT(review-by:…)` code marker; monthly janitor sweep. Box hygiene state (nightly Mongo+PG backups via `/usr/local/bin/fb-backup.sh`→`/srv/backups`; the CUDA-on-CPU backend-image bloat + slim draft in `backend-handoff/slim-image/`; open red flags = no mem_limits / no log rotation / runtime-only opencut bridge / only `:latest` tags) lives in memory `reference_hetzner-backups-and-hygiene.md`. See `feedback_deadweight-tagging-discipline.md`.
